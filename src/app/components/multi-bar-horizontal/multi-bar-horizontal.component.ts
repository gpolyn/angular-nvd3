...
@Component({
  selector: 'app-multi-bar-horizontal',
  templateUrl: './multi-bar-horizontal.component.html',
  styles: [style]
})
export class MultiBarHorizontalComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const url = BASE_URL + 'api/multi_bar_horizontal'; 
    ...
    if (this.parentNativeElement !== null) {
      ...
      this.d3Svg = d3ParentElement.select<SVGSVGElement>('svg');
			nv.addGraph(() => {
              ...
              .margin({left:100})
              .color(d3.scaleOrdinal<number, string>(d3.schemeCategory20).range());
          chart.x((d)=> { return d.label })
          chart.y((d)=> { return d.value })
          chart.yErr(function(d) { return [-Math.abs(d.value * Math.random() * 0.3), Math.abs(d.value * Math.random() * 0.3)] });
          chart.duration(250);
